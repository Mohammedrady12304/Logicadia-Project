...
export class AuthService {
  ...
  // دمج الـ apiUrl العام الخاص بزملائك مع إمكانية استخدامه المرن
  private apiUrl = 'https://localhost:7213/api';
  private tokenKey = 'token';

  // تسجيل الدخول المعدل لقراءة الـ Response بأي حالة أحرف وتأمين الـ Loop
    ...
    return this.http.post<any>(`${this.apiUrl}/auth/login`, credentials).pipe(
      tap(response => {
        ...
        if (isSuccess && token) {
          ...
          this.saveRole(role);
          // تأمين إضافي للـ userGuard القديم حتى لا يحدث تعارض في مفاتيح الكاش
          if (role) {
